 ...
 */
    ...
    useEffect(() => {
        ...
        setformValues(props.location.state.val)
...
/* axios part */
    ...
    const saveEmpData = async () => {
        console.log(props.location.state.val.empID);
        try {
            const res = await axios.post(`http://192.168.43.113:8080/editEmployee${props.location.state.val.empID}`,formValues)
            console.log(props.location.state.val.empID);
            console.log(res);
            if (res.data.error) {
        ...
        <>
            ...
            <div className='container my-5 w-25'>
                {Object.keys(formErrors).length === 0 && isSubmit ? (<div><Alert variant="success">
                    <Alert.Heading>Employee Edited Succesfully...!!!</Alert.Heading>
                </Alert></div>
                ) : (
                    <div></div>
                )}
